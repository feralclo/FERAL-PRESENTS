...
- **2026-05-03 — Server-side absolutize for promoter avatars/covers.** Promoter rows uploaded via the web admin's base64→`site_settings` path (handled by `/api/media/[key]`) store **relative** URLs (e.g. `/api/media/feral_promoter-avatar?v=...`). iOS couldn't construct a URL from these and rendered the initials chip everywhere FERAL appeared. New helper `lib/absolute-url.ts` rewrites root-relative URLs to absolute, preferring the request's `host` header (custom-domain tenants get their own host back) with `NEXT_PUBLIC_SITE_URL` as a fallback. Applied to all promoter-emitting endpoints: `/api/promoters/discover`, `/api/promoters/[handle]`, `/api/rep-portal/dashboard` (followed_promoters), `/api/rep-portal/me/following/promoters`, `/api/rep-portal/me/memberships`, `/api/rep-portal/reps/[id]` (teams array), and `/api/rep-portal/me/activity` (promoter_avatar_url). Absolute URLs (http/https/data:/`//host`) and Supabase Storage paths pass through unchanged. iOS surfaces the FERAL avatar on next deploy with no client change.
