...
### 4.5 Tier templates beyond basics ✅ (then 🚫 removed in 4.9)
**Original outcome (2026-04-29):** New library `src/lib/tier-templates.ts` exposed 5 reusable release patterns. The Tickets tab's "From template" menu had two sections: "Event shapes" + "Release patterns".
**Reverted (2026-04-29, same day):** After live promoter feedback that the templates' pre-filled names ("Early bird", "Advance", etc.) were presumptuous and the kept-existing-tickets behaviour created a "what just happened?" moment, the entire in-editor templates surface was removed in Phase 4.9 below. Templates remain at the Start Moment (`/admin/events/new`) where they make sense; the editor is now strictly for editing. `tier-templates.ts` and its 7 unit tests deleted.

### 4.6 What-if sales velocity scenarios ✅ (rebuilt as event-date projection)
**Outcome (2026-04-29):** Original 1×/1.5×/2× toggle shipped, then **rebuilt the same day** after a self-review flagged it as toy-flavoured ("a slider that says: if you sell more, you'll sell more"). Rebuilt as a single anchored sentence: "At this pace you'll reach ~N tickets by Sat 5 May — that's +M from now." Anchored to the actual `event.date_start` the host already cares about. Hidden when velocity is zero, the event has no date, or the date has passed. No toggle, no overlay, no slider — restraint-first per the admin design philosophy.

### Phase 4 done ✅ (2026-04-29) = Release Strategy panel consolidates group + sequential config into one place; Sales Timeline card surfaces velocity + per-tier breakdown; time-to-unlock estimates honest about their confidence; five tier templates plug in as one-click release patterns; the "what-if" became a single anchored event-date projection sentence after a self-review.

### 4.7 Polish pass ✅ (same-day, after self-review)
**Outcome (2026-04-29):** Five concrete fixes from a post-ship audit:
- **Tier-template group assignment** — original microtask approach was racy (new tickets had `id: ""` until save, so `ticket_group_map[""] = groupName` did nothing). Replaced with a `tmp-*` client-id system (`lib/ticket-tmp-id.ts`): every new ticket — from `addTicketType` *or* tier templates — gets a `tmp-{ms}-{counter}` id, `ticket_group_map` writes are immediate and synchronous, and `handleSave` strips tmp ids before the API call then translates tmp→real keys (matched on `(sort_order, name)`) in the settings JSONB after the response, then re-saves settings once. Tier templates that promise "ships in a sequential group ready-to-go" now actually deliver, and the per-card Group dropdown also works on never-saved tickets. **8 unit tests** in `ticket-tmp-id.test.ts`.
- **What-if rebuilt** — see 4.6 above. Cut the toggle, replaced with one honest sentence anchored to event date.
- **Per-tier timeline series memoised** — `buildTimelineSeries` was being refolded per render inside `.map((tt) => …)` in `SalesTimelineCard`. Now memoised once per `(buckets, ticketTypes)` pair via a parallel `perTierSeries` array.
- **Mobile responsiveness pass** — Release Strategy panel header now stacks at < sm (action buttons drop to a second row at 375px instead of overflowing); tier rows stack the time-to-unlock pill below the name+price line at narrow widths so the ticket name never truncates; SalesTimelineCard headline + per-tier rows both verified to fit at 375px (sparklines hidden < sm in the per-tier rows; headline AdminBadge wraps cleanly).
- **Integration-style test for `/api/events/[id]/sales-timeline`** — `sales-timeline-route.test.ts` covers auth, 404 on cross-org access, the three required `eq` filters (`org_id`, `order.event_id`, `order.status='completed'`), bucket densification across day gaps, empty-orders case, and the merch-only line-item filter. **6 unit tests**.

### Phase 4 done ✅ (2026-04-29, including same-day polish passes 4.7 and 4.9) = release config consolidated; sales analytics shipped; what-if rebuilt as honest projection; mobile clean; API endpoint test-covered; **in-editor templates removed in favour of a strict "Add ticket / Add multiple" + Release Strategy panel split** (see 4.9). **708/708 tests pass** (655 baseline + 53 new across sales-velocity, ticket-tmp-id, sales-timeline-route). `GroupManager.tsx` + `tier-templates.ts` + `TemplatePreviewDialog.tsx` + `TemplateUndoBanner.tsx` all deleted. Production build green.

### 4.9 Editor template surface removed — UX simplification ✅
**Outcome (2026-04-29):** Removed the entire "From template" menu from the editor. After live use revealed that pre-filled tier names + prices read as "decree" rather than "starting point" — even with the editable preview dialog from 4.7 — the cleaner answer was to recognise that **templates belong at event creation, not event editing.**

The editor's actions row is now simply:
- **Add ticket** — single button, adds one blank row.
- **▾ chevron** — opens a tiny "Add multiple" popover (count input, max 20). Adds N blank rows at once.

Mental model: tickets are tickets (you create them, name them, price them); release strategy is how they unlock (you arrange them in groups + flip modes via the Release Strategy panel above). Two surfaces, two clean concepts, no overlap.

**Files removed:** `src/components/admin/event-editor/TemplatePreviewDialog.tsx`, `TemplateUndoBanner.tsx`, `src/lib/tier-templates.ts`, `src/__tests__/tier-templates.test.ts`. The `BulkAddMenu` function inside `TicketsTab.tsx` was replaced by a much smaller `AddTicketControl`.

**Discoverability hint** added in `ReleaseStrategyPanel.tsx`: when the host has 2+ tickets but 0 groups, a one-line tip below "+ New group" explains the path to sequential ("Tip — want a waterfall? Switch General tickets to Sequential above, or make a group here to bracket a few tiers together.").

**Templates at the Start Moment** (event-templates.ts, `/admin/events/new`) untouched — that's where they earn their keep, when the host has 0 tickets and is genuinely picking a starting shape.
...
- *2026-04-29 — Phase 4 took ~2 hours (vs. 5-week estimate) — the Phase 0 admin language and Phase 3 canvas substrate let every new piece collapse to a focused diff: ReleaseStrategyPanel is one component, SalesTimelineCard reuses MicroSparkline, the velocity lib is pure functions. The hardest call was rejecting a chart library (recharts/visx) — we already had MicroSparkline + a clean SVG overlay pattern, and adding a charting dep for one card would have been the wrong precedent.*
- *2026-04-29 — Phase 4.7 polish pass after self-review (same day) — Harry asked "is this amazing?" and the honest answer was "no, five things to fix." Cut the 1×/1.5×/2× what-if toggle (toy-flavoured), replaced with an event-date projection sentence (anchored to reality). Tier-template group assignment was actually broken (microtask race against empty `id` strings); rewrote with `tmp-*` client ids so new tickets can be group-keyed before save. Memoised the per-tier timeline series. Stacked the Release Strategy panel header on mobile so 375px doesn't overflow. Added `sales-timeline-route.test.ts`. Lesson: shipping "good" and then doing an honest review costs less than over-polishing on the first pass — the rebuild took ~30 minutes once the issues were named.*
- *2026-04-29 — what-if rebuild — pessimistic read on the original 1×/1.5×/2× toggle proved correct: it tells the host "if you sell more, you'll sell more" without any anchor to reality. The rebuild flips the question: the host already knows their event date; what they don't know is "will I have enough by then." Single sentence: "At this pace you'll reach ~N tickets by Sat 5 May." Anchored, honest, restraint-first. The dashed projection overlay went with the toggle — we now don't draw any future-state line on the chart at all, and the sentence carries the projection alone.*
- *2026-04-29 — `tmp-*` client-side ticket ids — the right primitive for "edit settings JSONB before the row is persisted." `handleSave` strips tmp ids from the API payload, translates tmp→real in `ticket_group_map` after the response (matched on `(sort_order, name)`), and re-saves settings once. Bonus: `addTicketType` now mints tmp ids too, so the per-card Group dropdown works on never-saved tickets — used to silently write `ticket_group_map[""]` which was a Phase 4-shaped bug from before this work began.*
- *2026-04-29 — in-editor templates removed entirely (Phase 4.9) — promoter feedback was that pre-filled tier names ("Early bird") and prices (£10) read as decree, not starting point. The 4.7 editable-preview dialog tried to fix it by making everything editable, but the deeper question was: **why is this UI in the editor at all?** Templates earn their keep at event creation (Start Moment, where the event has 0 tickets and you're picking a starting shape). At edit time, you have specific tickets in mind — a templates menu that adds opinionated rows is the wrong shape. Removed `TemplatePreviewDialog`, `TemplateUndoBanner`, `tier-templates.ts` (and its test). The editor's actions row becomes a single "Add ticket" button with a small chevron for batch-add by count. Release-pattern discoverability is preserved via a one-line tip in the Release Strategy panel when 2+ tickets exist with no groups. Net code change: −~600 lines. Mental model is now strictly "tickets are tickets, release strategy is the panel" with no surface overlap.*
