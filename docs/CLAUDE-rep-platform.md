...
**Track suggestions** (`/api/rep-portal/spotify/suggestions`): four sections for the iOS picker. `recent` (rep's own picks 30d), `friends` (mutual follows 14d), `team` (same-promoter teammates 30d), `trending` (smart-mixed pool). Empty sections omitted. Reps never see genre labels — all curation lives in `src/lib/music/trending-playlists.ts` (platform-level, edit + ship). Pool refreshed every 6h by `cron/spotify-trending-refresh` (snapshot_id-aware: skips fetch when Spotify hasn't changed). Smart-mix (`src/lib/music/track-mix.ts`): popularity floor 15, fresh-window 14d boost + 7d `is_fresh` flag, cross-playlist co-occurrence boost, curator position weight, artist diversity (no two same-artist back-to-back), per-rep impression decay (drops at 5 unengaged shows). Affinity weighting (`deriveAffinity`) kicks in at 5+ rep picks — derived from which configured playlists the rep's picks live in; before that, even stratified sampling. Tables: `trending_playlist_snapshots`, `trending_track_pool`, `rep_track_impressions`. Spotify API limits respected (no `/recommendations` or `/audio-features` — both deprecated for new apps late 2024).
