...
| `src/components/admin/event-editor/GroupManager.tsx` | 🚫 | 2026-04-29 | Deleted (Phase 4.1) — group create/rename/delete/reorder + release-mode toggle moved into `ReleaseStrategyPanel`. |
| `src/components/admin/event-editor/TicketCard.tsx` | 🟨 | 2026-04-27 | Kept under `event-editor/` — still mounted by `TicketsTab`/`TicketsSection`. Per-card Group dropdown intentionally retained (cross-group reassignment is an inline action). Wrapper migration deferred. |
| `src/components/admin/canvas/sections/ReleaseStrategyPanel.tsx` | ✅ | 2026-04-29 | Phase 4.1 — single source of truth for ticket release config. AdminButton + AdminBadge wrappers; sequential mode shows numbered position chips + (Phase 4.4) hedged time-to-unlock estimates. Phase 4.9 added a one-line discoverability tip when ≥2 tickets exist with 0 groups. |
| `src/components/admin/canvas/sections/SalesTimelineCard.tsx` | ✅ | 2026-04-29 | Phase 4.3 + 4.6 — cumulative + daily MicroSparklines, headline velocity stat, memoised per-ticket-type rows, anchored event-date projection sentence (the original 1×/1.5×/2× toggle was rebuilt same-day after a self-review flagged it as toy-flavoured). |
...
| `src/components/admin/reps/QuestsTab.tsx` (editor → quest-editor) | ✅ | 2026-05-01 | Quest editor redesign Phase 4 — 5-tab inline form (≈1,500 lines + ~30 form-state hooks + Mux/Sharp upload pipeline) replaced with a single `<QuestEditor>` mount. New editor: 3-tile picker → one calm screen with `+ Add` chips → publish gate + tooltip-listed blockers + "You're live" sheet. List/table/filters/submissions untouched. File shrunk 2,044 → 877 lines. |
