...
function GroundGrid() {
  ...
  useFrame(({ clock }) => {
    if (gridRef.current) {
      ...
      mat.opacity = 0.2 + Math.sin(clock.elapsedTime * 0.3) * 0.04;
    }
  });
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < MAX_PARTICLES; i++) {
      if (i < particles.length) {
        ...
        const baseY = p.maxLife > 100 ? 0.5 : 0.35;
        ...
        return (
          <group key={stage}>
            ...
            <mesh position={[0, 0.01, z]} rotation={[-Math.PI / 2, 0, 0]}>
              ...
              <meshBasicMaterial color={meta.hex} transparent opacity={0.08} />
            </mesh>
...
function Atmosphere() {
  return (
    <>
      <ambientLight intensity={0.6} color={0x8888cc} />
      <directionalLight position={[10, 15, 10]} intensity={0.8} color={0xaaaadd} />
      <directionalLight position={[-8, 10, -5]} intensity={0.4} color={0x8b5cf6} />
      <pointLight position={[0, 8, 0]} intensity={0.5} color={0x8b5cf6} distance={30} />
      <fog attach="fog" args={[0x030306, 30, 65]} />
    </>
  );
}
