...
interface GenericAboutSectionProps {
  headingLine1: string;
  headingLine2: string;
  pillars: Pillar[];
  closer: string;
...
export function GenericAboutSection({
  headingLine1,
  headingLine2,
  pillars,
  closer,
}: GenericAboutSectionProps) {
  ...
  useEffect(() => {
    ...
    // Pillar animations
    const pillarEls = section.querySelectorAll(".about-pillar");
    ...
    pillarEls.forEach((p) => pillarObserver.observe(p));
    ...
    // Closer animation
    const closerEl = section.querySelector(".about-closer");
    const closerText = closerTextRef.current;
    if (closerEl && closerText) {
      ...
      closerObserver.observe(closerEl);
    }
  ...
  const numStr = (n: number) => String(n + 1).padStart(2, "0");
