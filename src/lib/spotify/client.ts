...
// Re-export from the dedicated module (kept here for backward-compat with
// any existing consumer that imports from this file).
export { pickBestAlbumImage } from "./album-image";
import { pickBestAlbumImage as pickBestAlbumImageInternal } from "./album-image";
...
function mapTrack(raw: RawSpotifyTrack): SpotifyTrack {
  ...
  const out: SpotifyTrack = {
    ...
    album: {
      name: raw.album?.name ?? "",
      image_url: pickBestAlbumImageInternal(raw.album?.images),
    },
